          ...
          productData.map((item, key) => {
            return <div key={item.id} className="flex items-center justify-between gap-6 mt-6">
              <div className="flex items-center gap-2">
                <CloseIcon onClick={() => dispatch(deleteFromCart(item.id))} className="text-xl text-gray-600 hover:text-red-600 cursor-pointer duration-300" />
                <img src={`${SystemConst.DOMAIN_HOST}/${item.productImage}`} className="w-32 h-32 object-cover" alt="" />
              </div>
              ...
              <p className="w-10">${item.unitPrice * item.quantity}</p>
            </div>
          })
        }
